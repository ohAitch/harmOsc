...
let model ?(delta = 1e-4) ?(z0 = 0.) ?(k = 1.) ?(mu = -. 1e-3) ?(points = 50000) init_val = 
  let t_prime (z, v, a) = 
    (z +. delta *. v , v +. delta *. a, (v *. mu) -. ((z -. z0) *. k) )
...
let r n = 10.**(Random.float n) in
  let delta = r (-4.) and mu = -. r (-2.) and pts = int_of_float (1000. *. r 2.) and amp = r 2. in
for iter = 1 to 50 do
  let k = exp((float iter) /. 5. -. 3.) in
    let out = open_out(sprintf "Sim%02d" iter
        (*"Sim%02d:k=%g__delta=%.1g__mu=%.1g__%d_points__starting_at_%.1g.data" iter k delta mu pts amp*)) in
